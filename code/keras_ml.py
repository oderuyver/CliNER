...
def train(train_X_ids, train_Y_ids, tag2id, vocab_size,
    ...
    # global variable so that it is only built once
    global lstm_model
    ...
    '''
    # analysis shows that maxlen=50 does not affect 99.95% of the training data
    # makes network WAY faster and also small enough to fit in memory on weaker computers
    small = [ s for s in train_X_ids if len(s) < 50 ]
    print len(train_X_ids)
    print len(train_X_ids) - len(small)
    diff = len(train_X_ids) - len(small)
    print float(diff) / len(train_X_ids)
    exit()
    '''

    # build model
    input_dim    = vocab_size + 1
    maxlen       = 50  # max(map(len, train_X_ids))
    num_tags     = len(tag2id)
    ...
    scores = {}
    scores['train'] = lstm_compute_stats('train', lstm_model, hyperparams,
                                         train_X, train_Y_ids)
    if val_X_ids:
        val_X = create_data_matrix_X(val_X_ids, len(val_X_ids), maxlen, num_tags)
        scores['dev'] = lstm_compute_stats('dev', lstm_model, hyperparams,
                                           val_X, val_Y_ids)
    #scores['history'] = history.history

    # make a list of the various hyper-parameters
    scores['hyperparams'] = {}
    scores['hyperparams']['input_dim'] = input_dim
    scores['hyperparams']['maxlen']    = maxlen
    scores['hyperparams']['epochs']    = epochs
...
def lstm_compute_stats(label, lstm_model, hyperparams, X, Y_ids):
    '''
    compute_stats()
    ...
    for i in range(nb_samples):
        ...
        ind = maxlen-num_words
        if ind<0: ind = 0
        tags = pred[i,ind:].argmax(axis=1)
        predictions.append(tags.tolist())

    scores = compute_performance_stats(label, predictions, Y_ids)
    return scores
...
def create_data_matrix_Y(Y_seq_onehots, nb_samples, maxlen, nb_classes):
    Y = np.zeros((nb_samples, maxlen, nb_classes), dtype=np.int32)
